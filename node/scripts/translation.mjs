...
function generateTs (schema, label) {
	...
	const valueImports = [
		...
		...unionNames.length ? [`\tSolveOptions as SolveOptionsMessage,`] : [],
	].join('\n')
	...
	const hasUnionFields = requestFields.some((f) => f.kind === 'union') ||
		Object.values(tables).some((fields) => fields.some((f) => unions[f.type]))
	...
	if (hasUnionFields) {
		...
		return generateTsUnion(schema, label, {
			valueImports, typeExports: typeExportsNoRequest, checks, responseFields,
			requestFields, unionNames, unionMemberClasses,
		})
	}
...
function generateTsUnion (schema, label, parts) {
	const { request, response, tables, unions, enums } = schema
	const { valueImports, checks, requestFields } = parts
	...
	const algorithmValues = algorithmEnum ? enums[algorithmEnum].values : {}
	...
	if (unionInfo && algorithmEnum) {
		...
		discriminatedUnion = `export type ${request} = {\n\tboxes: BoxTypeT[];\n\titems: ItemTypeT[];\n} & (\n${unionCases.join('\n')}\n${defaultCase}\n);`
		...
		const optionBuilders = unionInfo.members.map((memberName) => {
			...
			const args = fields.map((field) => `value.${toCamelCase(field.name)} ?? null`).join(', ')
			return `\tif (options?.${optionName}) {\n\t\tconst value = options.${optionName};\n\t\treturn { type: ${unionInfo.unionName}.${memberName}, value: new ${memberName}Object(${args}) };\n\t}`
		}).join('\n')
		const optionType = unionInfo.unionName
		const optionBuilder = `\nfunction toAlgoOptionsT(options: any): { type: ${optionType}; value: any } {\n${optionBuilders}\n\treturn { type: ${optionType}.NONE, value: null };\n}\n`
		const optionsFunction = optionBuilder
		parts.optionsFunction = optionsFunction
	}

	const noAlgoCheck = requestFields.filter((f) => f.enumName && f.defaultValue)

	const responseFields2 = resolveTable(response, tables, unions).map((f) => tsPlainObject('unpacked', f, schema)).join(',\n\t\t')
...
\t\t('algorithm' in request && request.algorithm !== undefined && request.algorithm !== null) ? request.algorithm : ${algorithmEnum}.${Object.keys(algorithmValues).pop()},
\t\toptionsT,
\t);
